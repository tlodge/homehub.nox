...
namespace vigil
{
    ...
    Disposition homework_dhcp::dhcp_handler(const Event& e) {
        ...
        struct iphdr *testip = (struct iphdr *) (*&reply + sizeof(struct ether_header));
        
        uint8_t *options = (uint8_t *)(*&reply + sizeof(struct ether_header) + 
    ...
    size_t homework_dhcp::generate_dhcp_reply(uint8_t **ret, struct dhcp_packet  * dhcp, 
        ...
        *ret = new uint8_t[len];
        ...
        ip->ttl = 0x80;
        ip->protocol = 0x11;
        uint32_t x = htonl(send_ip+1);
        char srcstr[INET_ADDRSTRLEN];
        inet_ntop(AF_INET, &x, srcstr, INET_ADDRSTRLEN);
       
        inet_pton(AF_INET, srcstr, &ip->saddr);
        inet_pton(AF_INET, "255.255.255.255", &ip->daddr);        
        ...
        options[0] = 1;
        options[1] = 4;
        //alignment over byte array issue - arm word length is 4 but at this point
        //we have written 5 bytes - so we are half way through the alignment which 
        //mashes stuff
        uint32_t nmask = htonl(0xFFFFFFFC); 
        ...
        options[0] = 3;
        options[1] = 4;
        uint32_t sendip = htonl(send_ip+1);
        options[2] = sendip;
        options[3] = sendip >> 8;
        options[4] = sendip >> 16;
        options[5] = sendip >> 24;
