...
namespace vigil
{
    ...
    Disposition homework_routing::device_handler(const Event& e) {
        printf("homework_routing event received.\n");
    ...
    Disposition homework_routing::mac_pkt_handler(const Event& e) {
        ...
        const Packet_in_event& pi = assert_cast<const Packet_in_event&>(e);
        Flow flow(pi.in_port, *(pi.get_buffer()));
        //lg.info("**** pkt_in packet: %s ******", flow.to_string().c_str()); 
        ...
        if(flow.dl_type == ethernet::ARP) {
            ...
            if( (flow.nw_proto == ip_::proto::UDP) && 
              ...
              //  lg.info("sending event to packet in handler");
        ...
        Disposition 
            ...
            if((is_src_local) && (ntohl(flow.nw_src)&0x3) == 1) {
                if ( (!this->p_dhcp->is_valid_mapping(ipaddr(ntohl(flow.nw_src)), flow.dl_src)) &&
                     ...
                     printf("received packet from unrecorded mac. "
        ...
        bool 
            homework_routing::send_flow_modification (Flow flow, uint32_t wildcard,  datapathid datapath_id,
                                                      ...
                                                      std::vector<boost::shared_array<char> > act) {
            //printf("sending a flow modification\n");
            ...
            //printf("AFTER dl src mac is %02x:%02x:%02x:%02x:%02x:%02x\n",  ofm->match.dl_src[0],ofm->match.dl_src[1],ofm->match.dl_src[2],ofm->match.dl_src[3],ofm->match.dl_src[4],ofm->match.dl_src[5]);
            
            //printf("AFTER dl dst mac is %02x:%02x:%02x:%02x:%02x:%02x\n",  ofm->match.dl_dst[0],ofm->match.dl_dst[1],ofm->match.dl_dst[2],ofm->match.dl_dst[3],ofm->match.dl_dst[4],ofm->match.dl_dst[5]);
            
            ofm->match.dl_type = flow.dl_type;    
            ofm->match.nw_src = flow.nw_src;
            ofm->match.nw_dst = flow.nw_dst;
            
            char srcstr_copied[INET_ADDRSTRLEN];
            inet_ntop(AF_INET, &ofm->match.nw_src, srcstr_copied, INET_ADDRSTRLEN);
            
            char dststr_copied[INET_ADDRSTRLEN];
            inet_ntop(AF_INET, &ofm->match.nw_dst , dststr_copied, INET_ADDRSTRLEN);
            
            //printf("AFTER match.nw_src is %s\n", srcstr_copied);
            //printf("AFTER match.nw_dst is %s\n", dststr_copied);
